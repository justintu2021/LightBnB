 ...
 */
 const getUserWithId = (id) => {
  return pool
    .query(`SELECT * FROM users WHERE user = $1;`, [ id ])
    .then((result) => result.rows[0])
    .catch((err) => err.message);
};
...
const addUser = (param) => {
  return pool
    .query(`INSERT INTO users(name,password,email) VALUES($1,$2,$3) RETURNING *;`, [ param.name, param.password, param.email ])
    .then((result) => {
      result.rows[0]})
    .catch((err) => err.message);
};
 ...
 */
 const addProperty = function(property) {
  ...
  const value = [property.owner_id, property.title, property.description, property.thumbnail_photo_url, property.cover_photo_url, property.cost_per_night, property.parking_spaces,
